...
import 'tinda_tracker/business_profiles_dao.dart';
import 'tinda_tracker/product_serial_numbers_dao.dart';
import 'tinda_tracker/product_recipe_ingredients_dao.dart';
...
class TindaTrackerDao {
  TindaTrackerDao(this._db)
    : productCategories = ProductCategoriesDao(_db),
      ...
      businessProfiles = BusinessProfilesDao(_db),
      productSerialNumbers = ProductSerialNumbersDao(_db),
      productRecipeIngredients = ProductRecipeIngredientsDao(_db);
  ...
  final BusinessProfilesDao businessProfiles;
  final ProductSerialNumbersDao productSerialNumbers;
  final ProductRecipeIngredientsDao productRecipeIngredients;
  ...
  Future<void> commitSale({
    ...
    List<ProductSerialNumbersCompanion> serials = const [],
  }) {
    return _db.transaction(() async {
      ...
      for (final sn in serials) {
        await productSerialNumbers.upsertLocal(sn);
      }
    });
  }

  /// Restocks one product: records the stock movement and adjusts the
  /// on-hand quantity atomically. Also registers new serial numbers if any.
